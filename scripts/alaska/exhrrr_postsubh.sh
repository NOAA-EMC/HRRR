...
# Script description:  This script post processes the sub-hourly data for the HRRR 
#
...
ln -sf ${PARMhrrr}/hrrrak_postcntrl_subh.xml postcntrl.xml
ln -sf ${PARMhrrr}/hrrrak_postxconfig_subh-NT.txt postxconfig-NT.txt
...
if [ $SENDCOM = YES ]
then
   cp ${POSTMGRDIR}/wrfsfc_${fhr}${fmin}.grib2 ${COMOUT}/hrrr.t${cyc}z.wrfsubhf00.ak.grib2
   $WGRIB2 ${COMOUT}/hrrr.t${cyc}z.wrfsubhf00.ak.grib2 -s > ${COMOUT}/hrrr.t${cyc}z.wrfsubhf00.ak.grib2.idx
   export err=$?;err_chk
fi

if [ $SENDDBN = YES ]
then
   $DBNROOT/bin/dbn_alert MODEL HRRR_AK_SUBH${ALERT_EXT} $job $COMOUT/hrrr.t${cyc}z.wrfsubhf${fhr}.ak.grib2
   $DBNROOT/bin/dbn_alert MODEL HRRR_AK_SUBH_WIDX${ALERT_EXT} $job $COMOUT/hrrr.t${cyc}z.wrfsubhf${fhr}.ak.grib2.idx
fi
fi
...
if [ $SENDCOM = YES ]
then
cat ${POSTMGRDIR}/wrfsfc_${fhrm1}15.grib2 ${POSTMGRDIR}/wrfsfc_${fhrm1}30.grib2 ${POSTMGRDIR}/wrfsfc_${fhrm1}45.grib2 ${POSTMGRDIR}/wrfsfc_${fhr}00.grib2 > ${COMOUT}/hrrr.t${cyc}z.wrfsubhf${fhr}.ak.grib2
export err=$?;err_chk

$WGRIB2 ${COMOUT}/hrrr.t${cyc}z.wrfsubhf${fhr}.ak.grib2 -s > ${COMOUT}/hrrr.t${cyc}z.wrfsubhf${fhr}.ak.grib2.idx
export err=$?;err_chk
fi
...
if [ $SENDDBN = "YES" ] ; then
       $DBNROOT/bin/dbn_alert MODEL HRRR_AK_GEMPAK_SUBH${ALERT_EXT} $job \
...
run91="00 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18"
...
cp ${COMOUT}/hrrr.${cycle}.wrfsubhf${fhr}.ak.grib2 hrrrak.${cycle}.ndfdsubhf${fhr}.grib2
$GRB2INDEX hrrr.${cycle}.ndfdsubhf${fhr}.ak.grib2 hrrrak.${cycle}.ndfdsubhf${fhr}.grib2i

  export FORTREPORTS=unit_vars=yes
  export FORT11=hrrrak.${cycle}.ndfdsubhf${fhr}.grib2
  export FORT12=hrrrak.${cycle}.ndfdsubhf${fhr}.grib2i
  export FORT51=xtrn_subh.${cycle}.hrrr${fhr}

  $TOCGRIB2 <$PARMhrrr/wmo/grib2_awips_hrrrak_subhf${fhr}.91  parm='KWBY' >> $pgmout 2> errfile 

  cp xtrn_subh.${cycle}.hrrr${fhr} ${WMO}/grib2.${cycle}.awphrrr_subh_91_f${fhr}_${cyc}
  if [ $SENDDBN_NTC = YES ]; then
    $DBNROOT/bin/dbn_alert NTC_LOW $NET $job ${WMO}/grib2.${cycle}.awphrrr_subh_91_f${fhr}_${cyc}
  fi
fi

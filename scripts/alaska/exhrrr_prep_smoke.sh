...
SENDCOM=${SENDCOM:-YES}
# MPI serial not needed on WCOSS2
...
RNPP_EXEC=$EXEChrrr/hrrr_smoke_proc_npp_frp_alaska
RMODIS_EXEC=$EXEChrrr/hrrr_smoke_proc_modis_frp_alaska
RNOAA20_EXEC=$EXEChrrr/hrrr_smoke_proc_j01_frp_alaska
FRP_EXEC=$EXEChrrr/hrrr_smoke_merge_frp_alaska
BBM_EXEC=$EXEChrrr/hrrr_smoke_fre_bbm_alaska
...
$CP -fp "\$5"*_mod_AK.txt $DATAHOME_PROC/.
EOF
...
${RUNSERIAL} ${BBM_EXEC} ${START_JULIAN}${HH}_frp3km.txt f${START_JULIAN}${HH}_bbm3km.txt ${MCD_FILE}
...
!---------------- For regional grids (polar or lambert)
   ...
   NNXP     = 1299,50,86,46,        ! Number of x gridpoints
   NNYP     = 919,50,74,46,        ! Number of y gridpoints
   ...
   POLELAT  = 60.8,           ! If polar, latitude/longitude of pole point
   POLELON  = -163.5,         ! If lambert, lat/lon of grid origin (x=y=0.)

   STDLAT1  = 60.0,           ! If polar for BRAMS, use 90.0 in STDLAT2
   STDLAT2  = 90.0,         ! If lambert, standard latitudes of projection
                            !(truelat2/truelat1 from namelist.wps, STDLAT1 < STDLAT2)
                            ! If mercator STDLAT1 = 1st true latitude
   STDLON   = -135.0,        ! Standard longitude, used if lambert or polar

   CENTLAT  =  60.8, -89.99,  -23., 27.5,  27.5,
   CENTLON  = -163.5,  -46.,-80.5, -80.5,
...
if [[ "$SENDCOM" == YES ]] ; then
    cp -fp FIRE-AK_new-T-${YEAR}-${MONTH}-${DAY}-${HOUR}0000-g1-bb.bin \
        $COMOUT/hrrrak.t${cyc}z.smoke.sources.bin
fi

${ECHO} "exhrrr_prep_smoke.sh.ecf completed at `${DATE}`"

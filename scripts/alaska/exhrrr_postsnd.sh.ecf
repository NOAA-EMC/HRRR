...
cp $PARMhrrr/hrrrak_bufr.tbl hrrr_bufr.tbl 
cp $PARMhrrr/hrrrak_modtop.parm hrrr_modtop.parm 
cp $PARMhrrr/hrrrak_sndp.parm.mono sndp.parm 
...
if test "$SENDDBN" = 'YES'
  then
   $DBNROOT/bin/dbn_alert MODEL HRRR_AK_BUFRSND${ALERT_EXT} $job $COMOUT/hrrr.t${cyc}z.class1.bufr.ak.${tmmark}
fi
...
cd ${DATA}/bufr.${cycle}
tar -cf - . | /usr/bin/gzip > ../hrrr.${cycle}.bufrsnd.tar.ak.gz
cd ${DATA}
if test "$SENDCOM" = 'YES'
then
   cp hrrr.${cycle}.bufrsnd.tar.ak.gz ${COMOUT}/hrrr.${cycle}.bufrsnd.tar.ak.gz
fi

#Send the alerts
if test "$SENDDBN" = 'YES'
  then
   $DBNROOT/bin/dbn_alert MODEL HRRR_AK_BUFRSND_TAR${ALERT_EXT} $job ${COMOUT}/hrrr.${cycle}.bufrsnd.tar.ak.gz
fi

# make gempak files
$USHhrrr/hrrrak_bfr2gpk.sh

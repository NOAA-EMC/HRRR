...
export FORMAT=rap.t%Hz.awp130bgrbf%f.grib2
export FORMAT2=rap.t[0-9][0-9]z.awp130bgrbf[0-9][0-9].grib2
export SOURCE=RAP
export WRF_CORE=ARW
...
if [[ ${ngribfiles} -ne $FILES_NEEDED && ${DELAY} -eq $DELAY_EXIT ]]; then
  echo "FATAL ERROR: Insufficient grib2 files for linking.  Need one of following full set of RAP files: \
       rap.t${cycm1}z.awp130bgrbfHH.grib2(.idx) where HH=01,04,07,10,13,16,19 \
       rap.t${cycm2}z.awp130bgrbfHH.grib2(.idx) where HH=02,05,08,11,14,17,20 \
       rap.t${cycm3}z.awp130bgrbfHH.grib2(.idx) where HH=03,06,09,12,15,18,21"
  err_exit
fi
...
#runline="aprun -n 168 -N 24 ./hrrr_wps_metgrid"
...
cp wrfbdy_d01 $COMOUT/hrrr.t${cyc}z.wrfbdy
cp rsl.out.0000 ${COMOUT}/hrrr.t${cyc}z.rslbcout

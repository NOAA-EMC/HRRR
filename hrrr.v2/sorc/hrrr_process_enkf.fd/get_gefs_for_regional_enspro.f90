...
!$$$ end documentation block
  ...
  use hybrid_ensemble_isotropic, only: nelen
 ...
 !use hybrid_ensemble_parameters, only: add_bias_perturbation
  ...
  type(sub2grid_info) grd_gfs,grd_mix,grd_arw
...
!  real(r_kind) :: ges_z (lat2,lon2  )
  ...
  real(r_kind) :: ges_u (lat2,lon2,nsig)
  real(r_kind) :: ges_v (lat2,lon2,nsig)
  real(r_kind) :: ges_tv(lat2,lon2,nsig)
  real(r_kind) :: ges_q (lat2,lon2,nsig)

  real(r_single),allocatable::ps_bar(:,:,:)
...
!mhu     if(.not.l_ens_in_diff_time) call stop2(85)
  end if
...
!  write(*,*) inner_vars,grd_ens%nlat,grd_ens%nlon,nsig_gfs,num_fields,regional
!  write(*,*) vector
...
!mhu     write(600+mype) ges_z(:,:)
!mhu     write(*,*) 'ges_z=',mype,maxval(ges_z),minval(ges_z)
...
!     if(mype==0) then
!     do k=1,grd_mix%nsig
!       write(*,*) 'T==',n,k,maxval(t_eg(:,:,k,n)),minval(t_eg(:,:,k,n))
!     enddo
!     endif
...
! Convert to mean
  ...
  allocate(ps_bar(grd_mix%lat2,grd_mix%lon2,1))
  do j=1,grd_mix%lon2
     do i=1,grd_mix%lat2
...
!www  ensemble perturbation for all but the first member if full_ensemble
  ...
  write(*,*) 'pbar_nmmb (cb) =',mype,&
            maxval(pbar_nmmb(:,:)),minval(pbar_nmmb(:,:))
...
!mhu  write(1000+mype) st_eg,vp_eg,t_eg,rh_eg,oz_eg,cw_eg,p_eg_nmmb
...
!  do k=1,grd_ens%nsig
!  write(*,*) 'ges_prsl_e=',mype,k,maxval(ges_prsl_e(1,:,:,k)),&
!                minval(ges_prsl_e(1,:,:,k))
!  write(*,*) 'ges_prsl=',mype,k,maxval(ges_prsl(:,:,k,1)),&
!                minval(ges_prsl(:,:,k,1))
!  enddo
...
!           do k=1,nsig
!             write(*,'(a,2I10,2F15.2)') 'check=',n,k,maxval(tt(:,:,k)),minval(tt(:,:,k))
!           enddo
...
!        write(*,*) mype,n,ic3,trim(cvars3d(ic3))
...
!        enddo
        ...
        call mpi_file_write(lunit,work,count,mpi_rtype,mpi_status_ignore,ierror)          
        if ( ierror /= 0 ) then
...
!     num_fields=nc3d*grd_ens%nsig+nc2d
     ...
     do n=1,n_ens
        if(mype==0) then
           ...
           if(mype==0) write(*,*) 'save perturbations for ', trim(filename)
           open(iunit,file=trim(filename),form='unformatted')

...
!
   ...
   start_index = 1
   end_index = 1
   print *, 'inside getvariable ', IM, IM1, JSTA_2L, JEND_2U
...
!
   ...
   print *, 'inside getvariableijk ', IM, IM1, JSTA_2L, JEND_2U, LM
   start_index = 1
   end_index = 1
    write(*,*)'fileName,DateStr,dh,VarName in getVariable= ',fileName,DateStr,dh,VarName
   call ext_int_get_var_info(dh,TRIM(VarName),ndim,ordering,Stagger,start_index,end_index,WrfType,ierr)
...
!    goto 27 avoiding this loop avoided failure
 ...
 27 continue
        print*, 'to deallocate of data'
   deallocate(data)

        print*, 'size(varbuff) in getVariableBikj: ', size(varbuff,dim=1), &
                    size(varbuff,dim=2),size(varbuff,dim=3)
   return

...
#  include "pnetcdf.inc"
  ...
  integer                                :: NumVars
  integer                                :: i
  integer                           :: info, ierr   ! added for GPFS -CLP (see NF_OPEN  below)
  ...
  if(Status /= WRF_NO_ERR) then
   ...
   call mpi_info_set(info,"romio_ds_read","disable", ierr) ; write(0,*)'mpi_info_set write returns ',ierr
   call mpi_info_set(info,'cb_buffer_size','4194304',ierr) ; write(0,*)'mpi_info_set write returns ',ierr
   call mpi_info_set(info,'striping_unit','4194304',ierr) ; write(0,*)'mpi_info_set write returns ',ierr


!  stat = NFMPI_OPEN(Comm, FileName, NF_NOWRITE, MPI_INFO_NULL, DH%NCID)
  stat = NFMPI_OPEN(Comm, FileName, NF_NOWRITE, info, DH%NCID)
...
! Comment out MPI-IO hints for other platforms and use following
! for NCEP.  To be changed to use compiler flags in future release.

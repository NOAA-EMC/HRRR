...
!             Set usage variable              
              ...
              if (sfctype) then  ! filter bad 2-m dew point and  0 mesonet wind obs
                 ...
                 if (qob .and. (kx >=180 .and. kx<=189) .and. obsdat(2,k) < 1.0e10_r_kind)  then ! for 2-m dew point
                    if(obsdat(12,k) < min(-40.0_r_kind,obsdat(3,k)-10.0_r_kind)) usage=116._r_kind     ! < -40C if T-Td>=10C                 
                    if((obsdat(3,k)-obsdat(12,k))  >  70.0_r_kind)  usage=117._r_kind ! <70C         
                    if(obsdat(12,k) > 32.2_r_kind) usage=118._r_kind  ! > 90F
                 endif
!                 if(abs(usage-115.0)<0.1) then
!                   write(*,*) 'toss meso wind obs=',obsdat(5,k),obsdat(6,k)
!                 endif
                 if((abs(usage -116.0) < 0.1) .or.  &
                    (abs(usage -117.0) < 0.1) .or.  &
                    (abs(usage -118.0) < 0.1)) then
                   write(*,*) 'toss 2mq obs=',usage,obsdat(3,k),obsdat(12,k),&
                            obsdat(2,k)
                 endif
              endif

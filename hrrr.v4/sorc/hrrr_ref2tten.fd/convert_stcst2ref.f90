SUBROUTINE convert_stcst2ref(nlon,nlat,nsig,ref_mos_3d,satcast_cr,h_bk)
!
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  convert_stcst2ref  convert satcast cloud top cooling to radar reflectivity
!
!   PRGMMR: Tracy L Smith          ORG: GSD/AMB        DATE: 2012-10-16
!
! ABSTRACT: 
!  This subroutine converts satcast cloud top cooling to radar reflectivity
!
! PROGRAM HISTORY LOG:
...
!     satcast_cr  - 2D satcast cloud top cooling rate in analysis grid
!     h_bk        - 3D height
!
...
!  INTEGER(i_kind),intent(in)   :: mype
  ...
  real(r_single), intent(in)   :: satcast_cr(nlon,nlat)
  real(r_single),   intent(inout):: ref_mos_3d(nlon,nlat,nsig)            ! reflectivity in grid
!
! local
!
  real(r_kind) :: dbz_satcast(nlon,nlat)
  real(r_single) :: satcast_ref(nlon,nlat)
  real(r_kind) :: table_lghtn2ref_winter(30)   ! table content the map from satcast cooling rates 
...
!  max reflectivity 45-50 dbz
  ...
  INTEGER(i_kind) :: num_satcast
  INTEGER(i_kind) :: i,j, k2, k, mref
  REAL(r_kind)    :: heightGSI,upref,downref,wght
  INTEGER(i_kind) :: ilvl,numref
  REAL(r_kind)    :: lowest,highest,tempref,tempprofile(maxlvl),tmpmos


!
! map cloud top cooling to maximum reflectiivty 
!
    write(*,*) 'starting convert_stcst2ref'
...
!         print*, 'satcast_cr ls -3',satcast_cr(i,j),i,j
        ...
        num_satcast = max(1,min(30,int(satcast_ref(i,j))))
        if(season== 2 ) then
           dbz_satcast(i,j) = table_lghtn2ref_winter(num_satcast)
        else if(season== 1 ) then
           dbz_satcast(i,j) = table_lghtn2ref_summer(num_satcast)
        endif
      endif
    ENDDO
  ENDDO
...
!         print*, 'dbz_satcast is gt 30', satcast_ref(i,j),dbz_satcast(i,j),i,j
         ...
         if(season== 2 ) then
           DO k=1,maxlvl
              tempprofile(k)=refprofile_winter(k,mref)*dbz_satcast(i,j)
           enddo
           lowest=newlvlAll(2)
           highest=7000.0_r_kind
         else if(season== 1 ) then
           DO k=1,maxlvl
              tempprofile(k)=refprofile_summer(k,mref)*dbz_satcast(i,j)
           enddo
           lowest=newlvlAll(3)
         ...
         DO k2=1,nsig
           ...
           if(heightGSI >= lowest .and. heightGSI < highest) then  ! lower 12km ?
              ...
              if(ref_mos_3d(i,j,k2) /= -99.0.AND.ref_mos_3d(i,j,k2) /= -99999.0) & 
                print*, 'ref_mos_3d before mod',ref_mos_3d(i,j,k2),i,j,k2
              tmpmos=-8888.0
              tmpmos=ref_mos_3d(i,j,k2)
              ref_mos_3d(i,j,k2) = max(ref_mos_3d(i,j,k2),tempref)
              if(ref_mos_3d(i,j,k2) /= tempref) then 
               if(tmpmos /= -99999.0.and.tmpmos /= -99.0) &
               print*, 'ne tempref',tmpmos,ref_mos_3d(i,j,k2),tempref,i,j,k2
!              print*, 'ref_mos_3d end of convSCst',ref_mos_3d(i,j,k2),i,j,k2, &
!                       tempref,dbz_satcast(i,j),mref
              endif
           endif
         ENDDO
      endif
...
END SUBROUTINE convert_stcst2ref

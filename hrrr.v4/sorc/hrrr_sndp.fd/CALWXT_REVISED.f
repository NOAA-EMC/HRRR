       SUBROUTINE CALWXT_REVISED(T,Q,P,PINT,LMH,LM,PPT,IWX)
C 
C     FILE: CALWXT.f
C     WRITTEN:  SEPTEMBER 2003, GEOFF MANIKIN 
C
...
C     BELOW FREEZING AND LOWEST LAYER TEMPERATURE.   THIS VERSION
C     DIFFERS FROM THE PRIMARY VERSION OF THE ALGORITHM IN THAT
C     THE AREA CHECK IS DONE TO FAVOR SNOW IN RESPONSE TO THE HIGH
C     BIAS OF THE ALGORITHM FOR SLEET AND FREEZING RAIN.
C 
...
c       IF (TWET(L).GE.269.15) AREAP4=AREAP4+AREA2
       PINTK1=PINTK2
 1945 CONTINUE
C
...
C             RAIN = 8
       ...
       ENDIF
C
C HERE IS WHERE IT VARIES.  THE OLD VERSION REQUIRES MINIMAL AREA
C   ON THE WARM SIDE OF -4 TO GET SNOW.  THE NEW VERSION GOES WITH
C   SNOW UNLESS THERE IS SUFFICIENT AREA ON THE WARM SIDE OF 0
C
C       IF (AREAP4.LT.3000.0) THEN
C             TURN ON THE FLAG FOR
C             SNOW = 1
C              IWX=1
C            GOTO 1900

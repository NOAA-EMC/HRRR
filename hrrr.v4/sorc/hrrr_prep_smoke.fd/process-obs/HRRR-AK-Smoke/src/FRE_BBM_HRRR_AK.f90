PROGRAM FRE_BBM_AK      ! Total biomass that burnt in kg
    IMPLICIT NONE
...
! Use is "./fre.exe input_fire output_text LULC_map Biome_map" (Ecosystem map); It covers the entire NA, 3km res.
...
!! IGBP LAND USE data
    INTEGER, PARAMETER ::  lulc_cols= 6040
    INTEGER, PARAMETER ::  lulc_rows= 4000

! Biome map
!    INTEGER, PARAMETER ::  biom_cols= 6040
!    INTEGER, PARAMETER ::  biom_rows= 4000

! Position in cartesian matrix
    INTEGER :: posix_lulc, posiy_lulc, stat
...
! LAND USE MAP (SAME AS READ MODIS AND VIIRS)
    ...
    IF (lulcmap.EQ.' ') THEN
       WRITE(*,*) 'Use: FRE <input frp daily file> <output file> <LULC> '
       STOP
    ENDIF
...
! Read the MAP with land use and land cover IGBP
    ...
    resx_lulc = 0.025   ! resolution of the Land-use map
    resy_lulc = 0.025
...
! Read the MAP with land use and land cover IGBP
    ...
    !resx_biom = 0.025   ! resolution of the biome map
    !resy_biom = 0.025
...
!RAR: new development is the separation of the Eastern temperate forest (array 2) and the rest of the North America (1)
    lu_sel:   SELECT CASE (INT(igbp))
                  CASE (0)                          ! water, here we assume fire near coastline e.g., needs further improvement
                     ...
                     bbcoef        = 1.2
                  CASE (1)                          ! Evergreen needleleaf, mostly SE Canada and western US
                     diur_dtime(1) = 7.71
                     diur_sdev(1)  = 4.73
                     diur_dtime(2) = 7.77
                     diur_sdev(2)  = 5.87
                     ...
                     bbcoef        = 1.55
                  CASE (2)                          ! Evergreen Broadleaf,  mostly the SE USA
                     diur_dtime(1) = 8.94
                     diur_sdev(1)  = 5.15
                     diur_dtime(2) = 6.32
                     diur_sdev(2)  = 4.89
                     ...
                     bbcoef        = 0.96
                  CASE (3)                          ! Deciduous Needleleaf, mostly SE Canada
                     diur_dtime(1) = 6.13
                     diur_sdev(1)  = 3.94
                     diur_dtime(2) = 8.10
                     diur_sdev(2)  = 6.25
                     ...
                     bbcoef        = 1.55
                  CASE (4)                          ! Deciduous Broadleaf, mostly eastern, NE USA, pats of Mexico
                     diur_dtime(1) = 5.77
                     diur_sdev(1)  = 2.62
                     diur_dtime(2) = 3.94
                     diur_sdev(2)  = 1.57
                     ...
                     bbcoef        = 1.55
                  CASE (5)                          ! Mixed forest, SE, NE, NW, SW USA, SE Canada
                     diur_dtime(1) = 6.48
                     diur_sdev(1)  = 4.18
                     diur_dtime(2) = 4.57
                     diur_sdev(2)  = 2.96
                     ...
                     bbcoef        = 1.55
                  CASE (6)                          ! Closed shrublands, mostly AR, NM, CA and OR
                     diur_dtime(1) = 8.14
                     diur_sdev(1)  = 5.28
                     diur_dtime(2) = 5.56
                     diur_sdev(2)  = 4.63
                     ...
                     bbcoef        = 0.78
                  CASE (7)                          ! Open shrublands, large parts of SW US and Mexico
                     diur_dtime(1) = 9.10
                     diur_sdev(1)  = 6.28
                     diur_dtime(2) = 6.17
                     diur_sdev(2)  = 4.30
                     ...
                     bbcoef        = 1.55
                  CASE (8)                          ! Woody savannas, SE USA, parts of Mexico, CA...
                     diur_dtime(1) = 9.81
                     diur_sdev(1)  = 6.12
                     diur_dtime(2) = 4.63
                     diur_sdev(2)  = 3.27
                     ...
                     bbcoef        = 1.55
                  CASE (9)                          ! Savannas, part of CA, TX and little bit in Canada
                     diur_dtime(1) = 8.70
                     diur_sdev(1)  = 5.74
                     diur_dtime(2) = 5.28
                     diur_sdev(2)  = 4.12
                     ...
                     bbcoef        = 0.78
                  CASE (10)                         ! Grassland, large fractions of western US, mid-west, TX
                     diur_dtime(1) = 7.26
                     diur_sdev(1)  = 4.82
                     diur_dtime(2) = 4.07
                     diur_sdev(2)  = 1.34
                     ...
                     bbcoef        = 0.78
                  CASE (11)                         ! Permanent wetlands, parts of LA, FL, SE USA, and SE Canada
                     diur_dtime(1) = 8.24
                     diur_sdev(1)  = 5.26
                     diur_dtime(2) = 5.38
                     diur_sdev(2)  = 4.01
                     ...
                     bbcoef        = 1.55
                  CASE (12)                         ! cropland, large fractions of mid-west, along Mississipi valley, south Canada, central valley CA, parts of ID, OR, WA and SE US, and Mexico
                     diur_dtime(1) = 3.00   !5.89   ! RAR: I set to 3 hours across the US
                     diur_sdev(1)  = 3.78
                     diur_dtime(2) = 3.00   !7.17
                     diur_sdev(2)  = 5.52   ! for cropland sigma isn't used
                  ...
                  CASE (13)                         ! Urban and Built-Up
                     ...
                     bbcoef        = 0.29
                  CASE (14)                         ! cropland/natural vegetation mosaic, some parts of NW, mid-west, east and SE USA, TX and Canada
                     diur_dtime(1) = 5.23
                     diur_sdev(1)  = 2.93
                     diur_dtime(2) = 4.15
                     diur_sdev(2)  = 1.64
                     ...
                     bbcoef        = 0.78
                  CASE (15)                         ! Snow and ice, very little in the domain
                     diur_dtime(1) = 6.42
                     diur_sdev(1)  = 3.94
                     diur_dtime(2) = 4.77
                     diur_sdev(2)  = 3.09
                     ...
                     bbcoef        = 0.49
                  CASE (16)                         ! Barren or sparsely vegetated, some parts of UT, NE, SE CA, NM and Mexico
                     diur_dtime(1) = 8.57
                     diur_sdev(1)  = 5.27
                     diur_dtime(2) = 4.77
                     diur_sdev(2)  = 4.70
...
! Simultaneous observations - assuming 1 minute delay
                ...
                IF (dbg) THEN
                    WRITE(*,*) 'n,= ',n
                    WRITE(*,*) 'igbp,diur_dtime= ',igbp,diur_dtime
...
! bbm is the total biomass burnt in kg, it's equal to total biomass x combustion efficiency (e.g. for grassland it's 100%)
             IF (acdsec < avg_fdt*3600.) THEN     ! short lasting fire
                IF ( (acdsec<10800.) .OR. (igbp == 0.) .OR. (igbp > 8.) ) THEN     ! If the detected duration is shorter than the diurnal cycle and it its igbp>10 or in the eastern US
                   totsec= avg_fdt*3600.
                ELSE
             ...
             ELSE    ! Otherwise do integration of FRP*dt, long lasting fires > diurnal cycle
                ...
                IF ( (igbp > 0.) .AND. (igbp <= 8.) ) THEN  ! forest and other classes in the western US, !!!grassland isn't included here!!!
                     totsec= 24.0*3600.0
                     bbm= bbcoef* (acfrp_t/n)* totsec           ! it's assumed that the forest and other fires last 24 hours
                ELSE
                     totsec= acdsec + 10800.   ! totsec here is calculated for diagnostic output only
                     bbm= bbcoef* fre_t        ! OTHER  LULC THAT HAVE TIME INTEGATION GREATER THAN DIURNAL CYCLE           ! 1.5 hour is added to the integration  to each tail.
                ENDIF
             ENDIF

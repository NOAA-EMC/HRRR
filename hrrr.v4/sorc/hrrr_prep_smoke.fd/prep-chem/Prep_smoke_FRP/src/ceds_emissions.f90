...
integer,save :: grid_byarea ! switch to perform grid by summing area emissions - ceds grid celss diced into 12X12
...
!--- for CEDS month is read in input (imon), only this month/year applied to global data
   imonx=1  
   grid_byarea=0
! No interpolation, only grid summing for fv3 and wrf model 
#ifdef WPS
    if(grid_type /= 'lat-lon')then
      grid_byarea=1
           ...
           do j=1,n2
           do i=1,n1
           grid_area(i,j) = 1./(grid_g(ng)%dxt(i,j)*grid_g(ng)%dyt(i,j))
!          grid_area(i,j)=deltax*deltay
          accum_emission = accum_emission + grid_area(i,j)
           enddo
           enddo
           write(*,*)'WPS type grid, area sum=',accum_emission
           call flush(6)
           endif
    endif
#endif
    if(grid_type == 'fv3' .or. grid_type == 'lat-lon')then
    grid_byarea=1
           accum_emission = 0.
           do j=1,n2
           do i=1,n1
          ...
          accum_emission = accum_emission + grid_area(i,j)
           enddo
           enddo
           write(*,*)'tile=',ng,', area sum=',accum_emission
           call flush(6)
    endif
...
! Read the data.  
 ...
 if(trim(spc_name(ispc)) == 'CH4')then ! CEDS anthro methane starts in 1970, not 2000
      ...
      call flush(6)
  if(grid_byarea == 1 .or. grid_type == 'fim')then
        do j=1,nlatX
        jc=int((j-1)/nXfv3)+1
        do i=1,nlonX
        ic=int((i-1)/nXfv3)+1
...
do i=1,n1
  ...
  if(grid_byarea == 1)then
        ...
        nmonths,nspecies,ilonnX,ilatnX,latcedsX,longcedsX,RAWsrc,grid_area,tx)
...
!-------------------- calculate the grib box area, if necessary--------------------------------------!
  if(allocated(grid_area))deallocate(grid_area)
allocate(grid_area(n1,n2))

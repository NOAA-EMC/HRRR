...
subroutine process_bbbem(iyear,imon,iday,ihour,ng,ngrids,n1,n2,n3,rlat,rlon,rland&
  ...
  character cjday*3, &
       ...
       cday*2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Modificacao para ler a data e hora direto do nome do arquivo
! Gabriel, 08/03/2017
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! Variaveis idate_abba, dum(5),itime_abba
...
110 continue
  ...
  do i=1,ic; if(qarea(i) < 1.e-6) qarea(i)=mean_area; enddo
   ...
   subroutine read_abba(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qtime,ic,itime_abba,nidate_abba,nitime_abba)
     ...
     ! ALTERADO PARA LER DO ARQUIVO E NAO DO CABECALHO
...
110  continue
   ...
   !------------------------------------------------------------------------------------------------------
  ! subroutine read_fre(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qfre,qtime,ic,dqfrpw,qigbp,dqsize)

   subroutine read_fre(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qfre,qtime,ic,dqfrpw,dqsize)

    !  Long               Lat           Average_FRP    Average_Fsize    Std_FRP
    !  Std_Fsize         Skewness       FsizeTotal      TPM              NOBS
    !  IGBP
    ! -124.978401         61.463280     7.700000       0.001617         1.300000
    ! 0.001200         -1.671000       0.001617        3640.190430      1
    ! 8.0000
...
!
!---------------------------------------------------------------------------------------
!Nome            Unidade     DescriÁ„o
!---------------------------------------------------------------------------------------
!MediaFRP        MW          Valor mÈdio de FRP dos fogos detetados dentro da grade.
!DesvioFRP       MW
!MediaFsize      km2         ¡rea mÈdia dos fogos detetados na grade.
!DesvioFsize     km2
!MediaEC         kW/m2       Valor mÈdio da energia convectiva do fogo.
!FsizeTotal      km2         ¡rea total dos fogos detetados na grade.
!FRE             MJ          Energia radiativa do fogo (Representa o total de biomassa
!                             consumida pelo fogo).
!NOBS            -           N˙mero de fogos detetados dentro da grade.
!---------------------------------------------------------------------------------------
        stat = 0
	...
	!print*, 'qlon', 'qlat', 'qfrpw', 'dqfrpw', 'dqsize', 'qsize', 'qfre'
		...
		read(iunit, *,  end=110) qlon(ic),qlat(ic),qfrpw(ic), dummyReal, &
					 ...
					 qsize (ic),qfre  (ic), dummyInteger, dummyReal !qigbp (ic)
					 
!Long               Lat           Average_FRP        Average_Fsize       Std_FRP          Std_Fsize         Skewness         FsizeTotal      BB              NOBS         IGBP
...
100  continue
 ...
 !inicializa interface do hdf5
	call h5open_f(error)
        if(error==0)then
           !n√£o exibe mensagem de erro do hdf5 caso ocorra um
           call h5eset_auto_f(printflags, error)
           !abre arquivo com exten√ß√£o hdf5
        ...
        !fecha a inicializa√ß√£o do hdf5

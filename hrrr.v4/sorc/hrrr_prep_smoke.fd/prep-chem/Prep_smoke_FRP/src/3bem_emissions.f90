...
!lzhang, read new version nesdis/wf_abba
if ((iyear>=2017.and.jday>=192) .or. iyear>=2018)then
        do nfn=1,nfiles 
           open(iunit,status='old',file=fnames(nfn))
           call read_abba_new(iunit,ntfocos &
...
!            ,maxfiles,nfiles)
        print*,trim(bbem_modis_data_dir)//'*'//cyear//cjday//'*'  ! 10/20/17 Correct file names (now by julian day)
        call bbbem_filelist(fnames,itimes,trim(bbem_modis_data_dir)//'*'//cyear//cjday//'*'&
             ,maxfiles,nfiles)
     ...
     !
     ...
     else if(use_bbem .eq. 2)then           ! RAR: this is the approach we're using
        ...
        call bbbem_filelist(fnames,itimes,trim(bbem_fre_data_dir)//cyear//cjday,maxfiles,nfiles)

        do nfn=1,nfiles 
           open(iunit,status='old',file=fnames(nfn))
           print*,"opening file=",nfn,fnames(nfn)
           call read_bbm(iunit,ntfocos &
                , fire_sensor_gt_1km(QLONG)%fire_prop &     ! Long
                , fire_sensor_gt_1km(QLATI)%fire_prop &     ! Lat
                , fire_sensor_gt_1km(QSIZE)%fire_prop &     ! Avg. FRP
                , fire_sensor_gt_1km(QFRPW)%fire_prop &     ! Avg. fire size
                , fire_sensor_gt_1km(QFRE)%fire_prop &      ! BBM
                , fire_sensor_gt_1km(QTIME)%fire_prop &     ! Std. dev. fire size
                , nfocos_sensor_gt_1km    &                 !
                , fire_sensor_gt_1km(DFRPW)%fire_prop &
                , fire_sensor_gt_1km(DSIZE)%fire_prop    )
           close(iunit)

! read_bbm(iunit,n,qlon,qlat,qsize,qfrpw,qfre,qtime,ic,dqfrpw,dqsize)
     ...
     call get_emission_per_fire_count(nfocos_sensor_gt_1km&
                                     , qsc,qfires                           &
                                     , fire_sensor_gt_1km(QLONG)%fire_prop  &   ! qlon
                                     , fire_sensor_gt_1km(QLATI)%fire_prop  &   ! qlat
                                     , fire_sensor_gt_1km(QAREA)%fire_prop  &   ! qarea
                                     , fire_sensor_gt_1km(QFRPW)%fire_prop  &   ! qfrpw
                                     , fire_sensor_gt_1km(QFLAM)%fire_prop  &   ! qflam
                                     , fire_sensor_gt_1km(QVEG )%fire_prop  &   ! qveg
                                     , fire_sensor_gt_1km(QVEG_AGREG)%fire_prop &   ! qveg_agreg
                                     , fire_sensor_gt_1km(QFRE)%fire_prop  &        ! BBM
                                     , veg_type_data_dir,carbon_density_data_dir&   ! veg_type_data_dir
                                     , fuel_data_dir)                               !
...
subroutine read_modis(iunit,n,qlon,qlat,qarea,qsize,QFRPW,qtime,ic)
  ...
  !do i=1,nobs1
     read(iunit,'(1X)',end=110)  ! 10/20/17 Read header line of MODIS file
  do i=1,n
     ic=ic+1
     !read(iunit,*,end=110) itime_modis_foco(ic),qlon(ic),qlat(ic)!,QFRPW(ic)
!    read(iunit,*,end=110) qlon(ic),qlat(ic)!,QFRPW(ic)
     read(iunit,*,end=110) qlat(ic),qlon(ic)! 10/20/17 MODIS files go lat,lon,brght,scan,trac,date,time....
  enddo
110 continue
...
110 continue
  ...
  do i=1,ic; if(qarea(i) < 1.e-6) qarea(i)=mean_area; enddo
   ...
   !------------------------------------------------------------------------------------------------------
   subroutine read_abba_new(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qtime,ic,itime_abba)
   implicit none
...
110  continue
ic = ic - 1   ! numero total de focos
print*,'=> read_abba_new,number of accumulated WF_ABBA fires=',ic
return
end subroutine read_abba_new
...
110  continue
   ...
   !------------------------------------------------------------------------------------------------------
   subroutine read_bbm(iunit,n,qlon,qlat,qsize,qfrpw,qfre,qtime,ic,dqfrpw,dqsize)

     implicit none
     integer,            intent(in)    :: iunit, n
     integer,            intent(inout) :: ic
     real, dimension(n), intent(out)   :: qlat,qlon, qsize, qfrpw, qfre, dqfrpw, qtime, dqsize
...
!     stat = 0
   ...
   ! FRP input data to prep_chem
   ! Long        Lat         Average_FRP    Average_Fsize    Std_FRP       Std_Fsize     BBM          NOBS     Duration       IGBP
   ! 68.544411   70.374863   7.127995       0.001568         1.781999      0.000392      361945.344   1        32760.00       7.00

     do i=1,n
           ic = ic + 1
           read(iunit,*,end=110) qlon(ic), qlat(ic), qfrpw(ic), qsize(ic), &
                                 dqfrpw(ic), dqsize(ic),  qfre(ic), dummyInteger, dummyReal, dummyReal

            write(*,100), ic, qlon(ic),qlat(ic),qfrpw(ic),qsize(ic),dqfrpw(ic),dqsize(ic),qfre(ic)

            100 format (1i4,06f18.6,1f20.3)
...
555     continue
   ...
   subroutine get_emission_per_fire_count(nfocos,qsc,qfires,qlon,qlat,qarea,qfrpw,qflam,    &
                                          qveg,qveg_agreg,qfre,veg_type_data_dir,           &
                                          carbon_density_data_dir,fuel_data_dir)
     ...
     ! The land use map is used to calculate the emissions   
     !-------------------------------------------------------------------------------
     data queiparms/  &
...
!KY           call define_bas_fuel_alaska(nfocos,bas_by_fire,iveg,qlat,qlon,fuel_data_dir)
...
!     if(use_bbem .eq. 1)then       ! RAR: we don't use it
...
!     	enddo
     ...
     IF (use_bbem .eq. 2) THEN       !- calculates the emissions by using the FRP data
     	do ifoc = 1,nfocos
           IF (iveg_agreg(ifoc) == 0) cycle         ! eliminates false detection of fires (over ocean/water bodies/iced land)
               ...
               WRITE(*,*) 'biomass_burned: ', biomass_burned
   ...
   subroutine interp_to_model(ng,n1,n2,n3,xt,yt,xm,ym,deltax,deltay,plat,plon,rlat&
	...
	if(iveg_ag < 1 .or. iveg_ag > 4) cycle ! incompatable place for fires
        ...
        elseif(grid_type == 'lambert' .or.grid_type == 'mercator' .or. &
        grid_type == 'lat-lon') then              !tks
           xlon=qlon(ifoc)                          !tks
           if(wlon.lt.-180)then
              splon=wlon+360.                        !tks
...
#ifdef WPS
           call lltoxy(qlat(ifoc),xlon,irbox,jrbox,0)
           igbox=INT(irbox+.5)
           jgbox=INT(jrbox+.5)
!   write(*,*)'ifoc,qlat,xlon,igbox,jgbox=',ifoc,qlat(ifoc),xlon,igbox,jgbox
     if(igbox .lt. 1 .or. igbox .gt. n1)igbox=-1
     if(jgbox .lt. 1 .or. jgbox .gt. n2)jgbox=-1
!          call lltoxy(29.32443,-124.3560,irbox,jrbox,0)
!          print *,'irbox,jrbox=',irbox,jrbox
!          call flush(6)
!          stop'999'
#endif
	...
	!------------
        ...
        do i=1,n1
 	      ...
 	      !- convert from mass consumed during the flaming phase to fraction of the total mass consumed
              do iesp=1,nspecies_actual
                 if(bbbem_g(iesp)%src(i,j,1) > 0.) &
                   !- fraction consumed at the flaming phase:
                   bbbem_plume_g(iesp,iveg_ag_ptr)%src(i,j,1)= bbbem_plume_g(iesp,iveg_ag_ptr)%src(i,j,1) / bbbem_g(iesp)%src(i,j,1)
              enddo
           enddo
        enddo
       endif
     ...
     elseif( grid_type == 'lat-lon') then   
     grid_area(:,:)=rland(:,:)  ! 4/22/18 Hijacked land variable for area in Cassini grid
     elseif(grid_type == 'll') then
        call get_area_ll(grid_area,n1,n2) 
     elseif(grid_type == 'fim') then
        call get_area_fim(grid_area,n1,n2) 
     elseif(grid_type == 'fv3') then
!       call get_area_fv3(grid_area,n1,n2) 
     grid_area(:,:)=rland(:,:)  ! 10/20/17 Hijacked land variable for area in FV3
     elseif(grid_type == 'gg') then
        call get_area_gg(grid_area,n1,n2,rlat,rlon) 
     else
        stop 'unknown grid type at 3bem emissions '
     endif
     write(*,*)'In 3bem_emissions, grid_area(1,1)=',grid_area(1,1)
...
!KY  call readOlsonHdf(nLon, nLat, olsonData)
...
100  continue
   ...
   !---------------------------------------------------------
   subroutine get_ij_fv3(n1,n2,rlat,rlon,lon_fire,lat_fire,igbox,jgbox)
     use grid_dims_out, only:  grid_resolucao_lon,grid_resolucao_lat
     implicit none
     integer, intent(in) ::n1,n2
     real, intent(in) :: rlat(n1,n2), rlon(n1,n2),lon_fire,lat_fire
     ...
     real :: mindist,currdist
     integer :: i,j
!    Stop'Endodaline get_ij_fv3'
     mindist=1.e9
     igbox=-1
     jgbox=-1
     do j=1,n2
     do i=1,n1
        if(abs(lon_fire-rlon(i,j)) .gt. 2. .or.&
             abs(lat_fire-rlat(i,j)) .gt. 2. ) cycle
...
!KY      subroutine define_bas_fuel_alaska(nfocos,bas_by_fire,qveg,qlat,qlon,ofn)
!KY        implicit none
!KY        include 'netcdf.inc'
!KY        integer nveg,nfocos,i,ifoc,jfoc,j,var_id,ncid
!KY        real,    dimension(nfocos) :: qlat,qlon,bas_by_fire
!KY        integer, dimension(nfocos) :: qveg
!KY        integer, parameter :: nlon2= 3111
!KY        integer, parameter :: nlat2=1807
!KY        real, save, dimension(nlon2,nlat2) :: rcarb_dens
!KY        real tmp
!KY        integer, save :: ncall
!KY        data ncall /0/
!KY        character (len=*) ofn
!KY
!KY        if(ncall == 0) then
!KY           ncall =1000
!KY           print*,'---------- Fuel load for Alaska-----'
!KY           print*,'opening:',TRIM(ofn); call flush(6)
...
!KY           call check( nf_open(TRIM(ofn), NF_NOWRITE, ncid)) 

           ! Get the varid of the data variable, based on its name.
!KY           call check( nf_inq_varid(ncid, "above_ground_biomass", var_id) )
           ! Read the data.  
!KY           call check( nf_get_var_real(ncid, var_id, rcarb_dens ) )
           ! Close the file, freeing all resources.
!KY           call check( nf_close(ncid) )
           !- invert array 
!KY           do j=1,nlat2/2
!KY              do i=1,nlon2
!KY                 rcarb_dens(i,j)         =tmp
!KY                 rcarb_dens(i,j)         =rcarb_dens(i,nlat2-j-1)
!KY                 rcarb_dens(i,nlat2-j-1) = tmp
!KY              enddo;enddo
!KY           endif
...
!KY              ifoc = nint(112.0273504*(qlon(i) -  (-1.677276786E+002   + 0.) )) + 1 !22.19 = 1/0.04505
!KY              jfoc = nint(112.0273504*(qlat(i) -  ( 5.650446429E+001  + 0.) )) + 1
!KY              ifoc=max(1,min(ifoc,nlon2))
!KY              jfoc=max(1,min(jfoc,nlat2))
              !   if(rcarb_dens(ifoc,jfoc) > 0.) then
              !     print*,ifoc,jfoc
              !     print*,qlat(i),qlon(i),rcarb_dens(ifoc,jfoc),bas_by_fire(i),qveg(i)
              !   endif
...
!KY           enddo
!KY         end subroutine define_bas_fuel_alaska
         !------------------------------------------------------------------------------------------------------

...
!           dlondeg = 360. / real(ixdim(1)-1)   ! Here, we really do not want e_we-s_we+1
!           dlatdeg = 180. / real(jydim(1)-1)   ! Here, we really do not want e_we-s_we+1
!           known_x = 1.
!           known_y = 1.
!           known_lon = stand_lon + dlondeg/2.
!           known_lat = -90. + dlatdeg/2.
!           dxkm = EARTH_RADIUS_M * PI * 2.0 / real(ixdim(1)-1)
!           dykm = EARTH_RADIUS_M * PI       / real(jydim(1)-1)
            dlatdeg = dykm
            dlondeg = dxkm
...
!           known_y=known_y+0.5
         ...
         call map_set(iproj_type, proj_stack(1), &
                      ...
                      lat1=known_lat, &
                      lon1=known_lon)
   write(*,*)'map_set call for Cassini,latinc,loninc,dx,dy=',dlatdeg,dlondeg,dxkm,dykm
   write(*,*)'stdlon,knowni,knownj,lat0,lon0=',stand_lon,known_x,known_y,pole_lat,pole_lon
   write(*,*)'lat1,lon1=',known_lat,known_lon
   call flush(6)
         ...
         call map_set(iproj_type, proj_stack(1), &
                      ...
                      loninc=dxkm, &
                      stagger=HH)
   write(*,*)'ixdim,jydim,phi,lambda,lat1,lon1,stagger=',ixdim(1),jydim(1),phi,lambda,known_lat,known_lon,HH
   ...
   write(*,*)'No nesting of rotated lat/lon available'
   STOP'in compute_nest_locations'

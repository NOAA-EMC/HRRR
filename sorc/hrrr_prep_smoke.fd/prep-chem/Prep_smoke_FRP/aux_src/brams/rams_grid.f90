...
if (num == 1) then
   ...
   do ifm = 1,ngrids
      ...
      elseif (ihtran == 4) then 
         print*,'IHTRAN value=4: no call to grdspc_ routine, expect bomb?'
      elseif (ihtran == 5) then 
         print*,'IHTRAN value=5: no call to grdspc_ routine, expect bomb?'
     ...
     if (ihtran == 3 .or. ihtran == 4 .or. ihtran == 5) goto 1000   ! no transformation for Mercator projection
...
do j = 1,nnyp(ngrid)
   do i = 1,nnxp(ngrid)
      ...
      print*," check grdspc_mc " 
      ! RSS - provavel causador do bug no mercator

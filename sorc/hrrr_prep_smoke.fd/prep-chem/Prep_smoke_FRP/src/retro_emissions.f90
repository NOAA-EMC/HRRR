...
integer :: dutraG
integer :: Istat
integer, dimension(5) :: pontos_MRJ  ! Angel 290516 - points for each 5 types of roads
...
real :: NOx_original_asc     !Angel DC 010616
real, dimension(5) :: porcentage    !Angel 290516 - emision percentage x road type
...
100 CONTINUE
        ...
        do j = 1, n2
   	        do i = 1, n1
		        retro_g(CO)%src(i,j,1)   = retro_g(CO)%src(i,j,1)*2 
            end do
        end do
		...
		do j = 1, n2
   			do i = 1, n1
                ...
                NOx_original_asc = NOXAlonso(i,j)      !Angel DC 010616

				!COAlonso(i,j)  = ((COAlonso(i,j)*1e7)/365.) / grid_area(i,j)  ! kg/m2/dia
				NOXAlonso(i,j) = ((NOXAlonso(i,j)*1e7)/365.) / grid_area(i,j) ! kg/m2/dia

                COAlonso(i,j)  = 2*(((COAlonso(i,j)*1e7)/365.) / grid_area(i,j))  ! kg/m2/dia    !Prueba 260716
                !NOXAlonso(i,j) = 0.6*(((NOXAlonso(i,j)*1e7)/365.) / grid_area(i,j)) ! kg/m2/dia
				...
				if(NOXAlonso(i,j) .gt. 0)then 
					...
					do ispc=1,nspecies
						if (ispc.ne.NOX .and. ispc.ne.CO .and. retro_g(NOX)%src(i,j,1) .ne. 0.) then
                               ...
                               !                         retro_g(ispc)%src(i,j,1) = (  NOXAlonso(i,j) / antiga_emision_nox(i,j,1) )  * retro_g(ispc)%src(i,j,1) ! nox ratio  !Angel DC 020616 
						end if 
					end do
		...
		deallocate(NOXAlonso, COAlonso)
   		deallocate(grid_area)
               !deallocate(antiga_emision_nox)    ! Angel DC 020616

	endif ! if(trim(user_data_dir(1:len_trim(user_data_dir)) ) /= 'NONE'


    !############# Rio de Janeiro (RJ) emissions georefered by roads ############
    if (trim(streets_inventory_region) == "RJ") then

        print*,'----------------------------------------------------------------------'
        print*,'Using local RJ inventory (JO)'
        print*,'Grid: ',ng
        ...
        !## Number of points for each type of road where the emissions will be distributed.
        ...
        !## Emissions to the municipalities of Rio de Janeiro (10 ^ 7 kg / year)

        CO_MRJ = 5.56    ! Para la prueba 2*CO 260716 
        !CO_MRJ = 2.78 
        !NOX_MRJ = 0.6*1.76   ! Para la prueba 0.6NOx 260716    
        NOX_MRJ = 17.6   ! Para la prueba 10NOx que falou Karla 010816    
        !NOX_MRJ = 1.76  

        !## Percentage of emissions for each type of road.
        ...
        !## Cycle to include emissions data on roads by type.    

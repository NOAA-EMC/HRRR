...
subroutine convert_megan_to_racm_reac(isp,iespc,ident,spc_name)
!use chem1_list
...
!---------------------------
!ISO (m=68) 
!ISOPRENE (m=68) ( Agg_RACM=1) (Agg=1)
...
!
!API (m=68) 
!API (m=136) 0.5*terpenes	=>	API  (Agg_RACM=1) (Agg=1)

if(spc_name == 'API') then
   ident = MONOTERPENES
   iespc=iespc+1
   emiss_spc_name(iespc,bioge)           = spc_name
   emiss_g(iespc)%src_bioge(:,:,1) = 0.5*megan_g(MONOTERPENES)%src(:,:,1)    
   found_emiss_spc(iespc,bioge)    = 1
   print*,'==> converted from megan - found for ',spc_name
   return
endif
...
!
!LIM(m=68) 
!LIM (m=136) 0.2*terpenes	=>	LIM  (Agg_RACM=1) (Agg=1)
...
if(spc_name == 'LIM') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bioge)           = spc_name
   emiss_g(iespc)%src_bioge(:,:,1) = 0.2*megan_g(MONOTERPENES)%src(:,:,1)   
   found_emiss_spc(iespc,bioge)    = 1
   print*,'==> converted from megan - found for ',spc_name
   return
endif
...
if(spc_name == 'HC3') then
   ...
   iespc=iespc+1
   emiss_spc_name(iespc,bioge)           = spc_name
   emiss_g(iespc)%src_bioge(:,:,1) = 0.49*megan_g(CH3OH)%src(:,:,1) + &
                                     0.57*megan_g(C3H8)%src(:,:,1)                                         
   found_emiss_spc(iespc,bioge)    = 1
   print*,'==> converted from megan - found for ',spc_name
   return
endif
...
!
!- ACETONE	=> KET  (Agg_RACM=0.33) 
!- OTHKETONES	=> KET  (Agg_RACM=1.61) 
...
end subroutine convert_megan_to_racm_reac

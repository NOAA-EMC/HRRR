...
!KY    integer, parameter :: nspecies_wrf = 29
    integer, parameter :: nspecies_wrf = 6
...
!KY      fileAB=trim(Fprefix)//file_in(1:len_trim(file_in)-7)//'-'&
!KY      //trim(spc_name_wrf(ispc_wrf))//'-ab.bin'
!KY	 open (91,file=fileAB,form='unformatted')
!KY	 print*, 'writing AB specie= ',spc_name(ispc), maxval(dummy), minval(dummy),iv+1
...
enddo
!KYprint*,'Anthro + biogenic emissions for WRF-Chem are in the file: ',fileAB
print*,"-------------------------------------------------------------"
...
ename='XXXXXXXX'
itime = 0
!KYfileAB=file_in(1:len_trim(file_in)-4)//'-ab.bin'

!KYopen (91,file=fileAB,form='unformatted')
!KY!     write(91)nspecies_wrf + 8
!KY     write(91)nspecies_wrf + 8 + nspecies_wrf_extra
!KY     write(91)ename
!KY     write(91)itime
!KYiv=0
!KYdo ispc_wrf=1,nspecies_wrf
!KY  !print*,'spc_name_wrf(ispc_wrf)',spc_name_wrf(ispc_wrf)
!KY  ifound=0
!KY  do ispc = 1,nspecies
!KY     !if(spc_alloc(src,ispc) == 0) cycle
!KY
!KY     !print*,'YY    ',spc_name(ispc),spc_name_wrf(ispc_wrf)
!KY
!KY     if(trim(spc_name_wrf(ispc_wrf)) == trim(spc_name(ispc))) then
!KY     
!KY     	 if(trim(spc_name_wrf(ispc_wrf)) == 'BBURN2' .or. &
!KY	    trim(spc_name_wrf(ispc_wrf)) == 'BBURN3') then
!KY	    ifound=1
!KY	    cycle
!KY	 endif
!KY	 
!KY         !print*,'FOUND ',spc_name_wrf(ispc_wrf),spc_name(ispc)
!KY	 
!KY         !-for WRF/FIM, 'biogenic' emissions for CO is not included
!KY	 !- 
!KY	 if(trim(spc_name(ispc)) == 'CO') then
!KY	     dummy(:,:) = var2d(:,:,ispc,antro)
!KY	 else
!KY	     dummy(:,:) = var2d(:,:,ispc,antro)+var2d(:,:,ispc,bioge)
!KY	 endif
!KY	    
!KY	 if(trim(spc_name_wrf(ispc_wrf)) == 'URBAN2' .or. &
!KY	    trim(spc_name_wrf(ispc_wrf)) == 'URBAN3' .or. &
!KY	    trim(spc_name_wrf(ispc_wrf)) == 'OC'   .or. &
!KY	    trim(spc_name_wrf(ispc_wrf)) == 'BC'  ) then
!KY	    fx= 1.e+9  / 86400.	 !  ug/(m^2 sec)
!KY	 else
...
!KY  enddo
!KY  if(ifound == 0) then
!KY!   print*,'NOT FOUND, zero emission for ',spc_name_wrf(ispc_wrf),iv+1
!KY     print*,'not found, zero emission, no binary write ',spc_name_wrf(ispc_wrf)
!KY     dummy(:,:) =0.
!KY!    write(91) dummy
!KY     iv=iv+1
!KY     ifound=1
...
!KY         dummy(:,:) = var2d(:,:,ispc,antro)
!KY	 print*, 'writing AB specie= ',trim(spc_name(ispc)), maxval(dummy), minval(dummy)
...
! NUWRF EMK...Add extra GFEDV3 species
! SAM 3/5/18 Tack extra species onto end of binary file - convert_emiss needs modification to accomodate
do ispc_wrf=1,nspecies_wrf_extra
   ifound=0
   do ispc = 1,nspecies
      if(trim(spc_name_wrf_extra(ispc_wrf)) == trim(spc_name(ispc))) then
         dummy(:,:) = var2d(:,:,ispc,bburn)
	 print*, 'writing BB specie= ',trim(spc_name(ispc)), maxval(dummy), minval(dummy)
         ! NUWRF EMK...Change DM units from kg/m^2/day to kg/km^2/hour. We don't
         ! have a molecular weight for this category.
         if (trim(spc_name_wrf_extra(ispc_wrf)) == 'DM') then
            fx = 1.e6 / 24.
         else
            fx= 1.e+3 * 1.e6 / weight(ispc) / 24.! emission unit: mol/(km^2 hour)
         end if
   ...
   enddo
   if(ifound == 0) then
     print*,'not found, zero emission, no binary write ',spc_name_wrf_extra(ispc_wrf)
!     write(91) dummy
      iv=iv+1
      ifound=1
   endif
enddo
! NUWRF EMK END...Add extra GFEDV3 species

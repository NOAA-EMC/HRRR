...
! 3/25/18 SAM: formats of frp data has changed - If .daily file - one format.
! If a .frp file, another format (call read_fre2).
        dummy=fnames(nfn)
        j=index(dummy,' ')-1
        write(6,*)'j,dummy=',j,dummy
        call flush(6)
        if(dummy(j-4:j).eq.'daily')then
		...
		, fire_sensor_gt_1km(QTIME)%fire_prop &
           ...
           close(iunit)
        elseif(dummy(j-2:j).eq.'frp')then
           call read_fre2(iunit,ntfocos &
                , fire_sensor_gt_1km(QLONG)%fire_prop &
                , fire_sensor_gt_1km(QLATI)%fire_prop &
             !   , fire_sensor_gt_1km(QAREA)%fire_prop &
                , fire_sensor_gt_1km(QSIZE)%fire_prop &
...
110  continue
   ...
   !------------------------------------------------------------------------------------------------------
   subroutine read_fre(iunit,n,qlon,qlat,qsize,qfrpw,qfre,qtime, ic,dqfrpw,dqsize)
...
110 continue
     	ic = ic - 1   ! numero total de focos
     print*,'=> number of -daily- accumulated fre_pix fires=',ic
     return
   end subroutine read_fre
   !------------------------------------------------------------------------------------------------------
   subroutine read_fre2(iunit,n,qlon,qlat,qsize,qfrpw,qfre,qtime, ic,dqfrpw,dqsize)
     ...
     integer            :: dummyInteger
     real               :: dummyReal
     real, dimension(200) :: s_frp,s_dfrp,s_dsz,s_sz ! 200 max counts per view allowed in input file
     character(len=120) :: dummyString
!
!---------------------------------------------------------------------------------------
!Nome            Unidade     Descrição
...
!---------------------------------------------------------------------------------------
	...
	!print*, 'qlon', 'qlat', 'qfrpw', 'dqfrpw', 'dqsize', 'qsize', 'qfre'
        ...
        do i=1,n
	        ic = ic + 1
                read(iunit, *,  end=110) j
     qfre(ic)=j
     read(iunit, *,  end=110) qlon(ic), qlat(ic), (s_frp(k),k=1,j),  &
     (s_dfrp(k),k=1,j), (s_dsz(k),k=1,j), (s_sz(k),k=1,j)
      qfrpw(ic)=0.
      dqfrpw(ic)=0.
      dqsize(ic)=0.
      qsize(ic)=0.
      do k=1,j
      qfrpw(ic)=qfrpw(ic)+s_frp(k)
      dqfrpw(ic)=dqfrpw(ic)+s_dfrp(k)
      dqsize(ic)=dqsize(ic)+s_dsz(k)
      qsize(ic)=qsize(ic)+s_sz(k)
      enddo
      qsize(ic)=qsize(ic)/FLOAT(j)
!	read(iunit, *,  end=110) qlon  (ic),qlat  (ic), qfrpw(ic),  &
!				 dqfrpw(ic),dqsize(ic),        &
!				 qsize (ic),qfre  (ic) 
        	write(*,100)"FRE", ic, qlon(ic),qlat(ic),1.e+3*qfrpw (ic),1.e+6*qsize(ic),&
...
110 continue
     	ic = ic - 1   ! numero total de focos
     print*,'=> number of -frp- accumulated fre_pix fires=',ic
     return
   end subroutine read_fre2
   !------------------------------------------------------------------------------------------------------
...
2    format(4i5)
   ...
   subroutine le_veg(nfocos,NO,MOF,NP,DELTALLO,DELTAXP,DELTAYP,IBLKSIZO  &
     ...
     do ifoc=1,nfocos
      ...
      WRITE(6,'(A,7I6,2F11.4)')'ifoc,ISOC,ISOCPT,ISOCPO,IWOCPH,IWOCPT,IWOCPO,lat,lon=', &
      ifoc,ISOC,ISOCPT,ISOCPO,IWOCPH,IWOCPT,IWOCPO,glatp,glonp
      CALL FLUSH(6)
        IF(ISOC.GE.0)THEN
...
!    Stop'Endodaline get_ij_fv3'
 ...
 !inicializa interface do hdf5
	call h5open_f(error)
        if(error==0)then
           !nÃ£o exibe mensagem de erro do hdf5 caso ocorra um
           call h5eset_auto_f(printflags, error)
           !abre arquivo com extenÃ§Ã£o hdf5
        ...
        !fecha a inicializaÃ§Ã£o do hdf5

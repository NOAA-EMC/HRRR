...
110  continue
   ...
   subroutine read_bbm(iunit,n,qlon,qlat,qsize,qfrpw,qfre,qtime,ic,dqfrpw,dqsize)
     ...
     integer,            intent(in)    :: iunit, &
                                          n
     integer,            intent(inout) :: ic
     real, dimension(n), intent(out)   :: qlat,qlon, &
                                          qsize, &
                                          qfrpw, qfre, dqfrpw,  &
					                      qtime, &
                                          dqsize
...
! Files contain 'Long  Lat  Average_FRP  Average_Fsize   Std_FRP  Std_Fsize   Skewness    FsizeTotal  BBM   NOBS  Duration  IGBP' variables

     do i=1,n
	    ic = ic + 1
           read(iunit,*,end=110) qlon(ic), qlat(ic), qfrpw(ic), qsize(ic), &
                                 dqfrpw(ic), dqsize(ic), dummyReal,        &
                                 dummyReal, qfre(ic), dummyInteger, dummyReal, dummyReal

            write(*,100)"FRE:", ic, qlon(ic),qlat(ic),1.e+3*qfrpw (ic),1.e+6*qsize(ic),&
                               (1.e+3*qfrpw (ic))/(1.e+6*qsize(ic)), qfre(ic)
            100 format (1x,A3,1x,I4,1x,2F7.2,1x,2E12.3,1x,F7.3,F10.1)

...
module diurnal_cycle_edgar

contains 

subroutine cycle_edgar(ihour,rhour,iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltax,deltay&
                            ,xt,yt,xm,ym,plat,plon)
use grid_dims_out, only: grid_type,retro_data_dir
use edgar_emissions
use util_geometry
...
if (ispc.eq.1) then
edgar_g(ispc)%src(i,j,1)=(edgar_g(ispc)%src(i,j,1)*peso)/3600
endif
if (ispc.eq.2) then
edgar_g(ispc)%src(i,j,1)=(edgar_g(ispc)%src(i,j,1)*pesoN)/3600
endif
if (ispc.ne.1.and.ispc.ne.2) then
edgar_g(ispc)%src(i,j,1)=(edgar_g(ispc)%src(i,j,1)*peso)/3600
endif
...
end subroutine cycle_edgar
end module diurnal_cycle_edgar

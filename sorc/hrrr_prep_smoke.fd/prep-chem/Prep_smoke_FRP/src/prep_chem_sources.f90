...
!KY  call FV3_model_grid_type(ihour,iday,imon,iyear)
...
!-----------------------------------------------------------------------------
!KY subroutine FV3_model_grid_type(ihour,iday,imon,iyear)
!KY use an_header
!KY use mem_grid, only : nnxp, nnyp, nnzp, ngrids, nzg, npatch
!KY use grid_dims_out 
!KY implicit none
!KY   include 'netcdf.inc'
!KY integer, intent(in) :: ihour,iday,imon,iyear
!character (len=*), intent(in)  ::  chem_out_prefix

!KY integer :: i,j,iv,ng,nfn,nfiles,nvert
!KY integer :: ncid,stat,nDims, nVars, nAtts, unlimDimID,dval,id_var
!KY integer :: istart(2),iend(2)
!real*8, allocatable, dimension(:,:) :: d2d,lonsv  ! Double precision lat,lons in FV3 files
!KY real, allocatable, dimension(:,:) :: d2d,lonsv  ! Double precision lat,lons in FV3 files
!KY character (len=240) :: fnames(maxfiles),dname
!KY real, allocatable, dimension(:,:,:) :: r2d
!- for FV3 grid informations
!KY integer :: nprg,npoints
!KY integer, parameter :: lat =1 , lon =2 , land =3
!KY real*8  d, d2r, dist, distance
!KY logical              :: double_precision,there
!KY character(16)	     :: header
!KY integer :: fire_loc,nprox,prox,proxs,inv_perm,i3,isn,i2,i1,kk	! dummies, not needed
!KY real*8 xh(4) ! holding variable for longitude

!KY d2r = 4.0*ATAN(1.0)/180.0

!KY print*,'FV3 grid directory=',trim(fv3_data_dir)
!- determine the grid configuration
!KY ngrids = 6
!KY proj_to_ll='NO'
!KY nnzp(1:ngrids)=1
...
!KY   nfn=1
  ...
  !-lon/lat configuration
! inquire(file='./icos_grid_level.dat',exist=there)
!KY   WRITE(fnames(nfn),'(A5,I1,A)')'.tile',ng,'.nc'
!KY   inquire(file=trim(fv3_data_dir)//trim(fnames(nfn)),exist=there)
!KY   if(.not.there) then
!KY      print*,'file not found:', fnames(nfn)
!KY      stop 'at FV3_model_grid_type routine 1'
!KY   endif

!KY         stat = nf_open(trim(fv3_data_dir)//trim(fnames(nfn)), NF_NOWRITE, ncid)
!KY         print*,'ncid,stat=',ncid,stat
!KY   stat = nf_inq(ncid, nDims, nVars, nAtts, unlimDimID)
!KY   do i = 1, nDims
!KY     stat = nf_inq_dim(ncid, i, dname, dval)
!   if     (dname .eq. 'nx') then
!KY     if     (dname .eq. 'grid_xt') then
!KY     nnxp(ng)=dval
!   elseif (dname .eq. 'ny') then
!KY     elseif (dname .eq. 'grid_yt') then
!KY     nnyp(ng)=dval
!KY     endif
!KY   write(*,'(A,I,A,I,2A)')'dim,i=',i,' dval= ',dval,' name=',dname
!KY   call flush(6)
!KY   enddo
  !- allocate arrays for lat,lon,land
!KY   if( allocated (r2d)) deallocate (r2d)
!KY   allocate(r2d(nnxp(ng),nnyp(ng),3))
...
!KY   if( allocated (d2d))then
!KY   deallocate (d2d)
!KY   deallocate (lonsv)
!KY   endif
!KY   allocate(d2d(iend(1),iend(2)))
!KY   allocate(lonsv(iend(1),iend(2)))
! stat = nf_inq_varid ( ncid, 'x', id_var )
!KY   stat = nf_inq_varid ( ncid, 'grid_lon', id_var )
!KY   call ncvgt( ncid,id_var,istart,iend,d2d,stat)
!KY   do i=1,nnxp(ng)+1
!KY   do j=1,nnyp(ng)+1
!KY    if(d2d(i,j) .gt. 180.0) d2d(i,j)=d2d(i,j)-360. ! longitude from -180. to 180.
!KY   enddo
!KY   enddo
!KY   lonsv(:,:)=d2d(:,:)
!KY   do i=1,nnxp(ng)
...
!KY   xh(3)=d2d(i,j+1)
!KY   xh(4)=d2d(i+1,j+1)
!KY   if(maxval(xh)- minval(xh).gt.179.)then
!KY   r2d(i,j,lon)=0.
!KY   do kk=1,4
!KY   if(xh(kk).lt.0.)xh(kk)=xh(kk)+360.
!KY   r2d(i,j,lon)=r2d(i,j,lon)+0.25*xh(kk)
!KY   enddo
!KY   if(r2d(i,j,lon).gt.180.)r2d(i,j,lon)=r2d(i,j,lon)-360.
! write(6,'(A,2I7,1P5D15.8)')'dateline fix,i,j,d2ds,r2d=',i,j,d2d(i,j), &
! d2d(i+1,j),d2d(i,j+1),d2d(i+1,j+1),r2d(i,j,lon)
! call flush(6)
!KY   else
!KY   r2d(i,j,lon)=.25*(d2d(i,j)+d2d(i+1,j)+d2d(i,j+1)+d2d(i+1,j+1)) !grid center lon
!KY   endif
!KY   enddo
!KY   enddo

!KY   stat = nf_inq_varid ( ncid, 'grid_lat', id_var )
!KY   call ncvgt( ncid,id_var,istart,iend,d2d,stat)
!KY   do i=1,nnxp(ng)
!KY   do j=1,nnyp(ng)
!   r2d(i,j,lat)=d2d(i,j)
!KY   r2d(i,j,lat)=.25*(d2d(i,j)+d2d(i+1,j)+d2d(i,j+1)+d2d(i+1,j+1)) !grid center lat
!KY   enddo
!KY   enddo
!KY   call fv3_ltln_buff(iend(1),iend(2),d2d,lonsv,0) ! Buffer FV3 lat,lons into memory
! SAM 10/13/17, Hijack 3rd dimension of r2d (normally land) to be area in FV3 file
!KY   iend(1)=nnxp(ng)
!KY   iend(2)=nnyp(ng)
!KY   if( allocated (d2d)) deallocate (d2d)
!KY   allocate(d2d(iend(1),iend(2)))
!KY   stat = nf_inq_varid ( ncid, 'area', id_var )
!KY   call ncvgt( ncid,id_var,istart,iend,d2d,stat)
!KY   do i=1,nnxp(ng)
!KY   do j=1,nnyp(ng)
!KY     r2d(i,j,land)=d2d(i,j)
...
! endif  ! endof if(.not.there) then
  ...
  ! determine the output dimension and arrays:
  ! these are not accurate
!KY    nxgrads(ng)    = nnxp(ng)
!KY    nygrads(ng)    = nnyp(ng)	     
!KY    dep_glon(1,ng) = r2d(1,1,lon)
!KY    dep_glon(2,ng) = r2d(2,1,lon)-r2d(1,1,lon)
!KY    dep_glat(1,ng) = r2d(1,1,lat)
!KY    dep_glat(2,ng) = r2d(2,1,lat)-r2d(1,1,lat)
!KY    nxa(ng) = 1
!KY    nxb(ng) = nnxp(ng)
!KY    nya(ng) = 1
!KY    nyb(ng) = nnyp(ng)
  ...
  ! calculation of the sources emission:
  !nvert = nnzp(ng) !use this for 3d sources
!KY    nvert = 1        !use this for 2d sources
!KY   call prep_sources(ng,nnxp(ng),nnyp(ng),nnzp(ng) &
!KY                   ,r2d(:,:,1) & ! lat
!KY 		  ,r2d(:,:,2) & ! lon
!KY 		  ,r2d(:,:,3) & ! land
!KY                   ,ihour,iday,imon,iyear)
!STOP'EndodalineFV3_model_grid_type'
  !
  !- collect data that will be send out
!KY   call select_data(ng,nnxp(ng),nnyp(ng),nvert)
  
  
  !- performs the output at native and projected grids (for visualization, if required)
!KY   call output(ng,nvert,ihour,iday,imon,iyear,ngrids, nnxp(ng),nnyp(ng), nnzp(ng),5)
!KY enddo ! end of ng=1,ngrids loop

!KY   deallocate(r2d)

!KY end subroutine FV3_model_grid_type
!-----------------------------------------------------------------------------!-----------------------------------------------------------------------------
...
!------------------------------------------------------------
!  Biogenic  emission 
!KYif(use_bioge == 1) then
   !KYcall mem_bioge(n1,n2,n3)
   !KYcall read_bioge(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
    		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KY
!KYelseif(use_bioge == 2) then
   !KYcall mem_megan(n1,n2,n3)
   !KYcall read_megan(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
    		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
!  EDGAR anthropogenic emission 
!KYif(use_edgar == 1.or.use_edgar == 2.or.use_edgar == 3) then
   !KYcall mem_edgar(n1,n2,n3)
   !KYcall read_edgar_antro(ihour,local,iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
    		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
...
!KYif(use_gocart == 1) then
   !KYcall mem_gocart(n1,n2,n3)
   !KYcall read_gocart_antro(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
    		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
!  CEDS anthropogenic emission 
   !KYif(use_ceds == 1) then
   !KYcall mem_ceds(n1,n2,n3)
   !KYcall read_ceds_antro(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
   !KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
   !KYendif
!------------------------------------------------------------
...
!------------------------------------------------------------
!  volcanic emission 
!KYif(use_volcanoes == 1) then
   !KYcall mem_volcanoes(n1,n2,n3)
   !KYcall process_volcanoes(iyear,imon,iday,ihour,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
  		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
! degassing volcanic emission 
!KYif(use_degass_volcanoes == 1) then
   !KYcall mem_degassing_volcanoes(n1,n2,n3)
   !KYcall process_degassing_volc(iyear,imon,iday,ihour,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
  		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
!  GOCART Background
!KYif(use_gocart_bg == 1) then
   !KYcall mem_gocart_bg(n1,n2)
   !KYcall read_gocart_bg(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
    		!KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
!  AFWA Erodability factors
!KYif(use_afwa_erod == 1) then
   !KYcall mem_afwa_erod(n1,n2)
   !KYcall read_afwa_erod(iyear,imon,iday,ng,ngrids,n1,n2,n3,rlat,rlon,rland,deltaxn(ng)&
        !KY,deltayn(ng),xtn(1,ng),ytn(1,ng),xmn(1,ng),ymn(1,ng),platn(ng),plonn(ng))
!KYendif
!------------------------------------------------------------
...
if (trim(chemical_mechanism) == 'RACM') then 
   ...
   if (pond == 0) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_racm(isp,iespc,ident,spc_name)  
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_racm(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_racm(isp,iespc,ident,spc_name)		
   elseif (pond == 1) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_racm_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_racm_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_racm_reac(isp,iespc,ident,spc_name)
   endif
   return
elseif (trim(chemical_mechanism) == 'RADM_WRF_FIM'.or.trim(chemical_mechanism) == 'RADM_FV3') then
   ...
   if (pond == 0) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_wrf(isp,iespc,ident,spc_name)  
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_wrf(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_wrf(isp,iespc,ident,spc_name)		
      !KYif(trim(inventory_type) == 'ceds' )   call convert_ceds_to_wrf(isp,iespc,ident,spc_name)
...
!     endif
   elseif (pond == 1) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_wrf_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_wrf_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_wrf_reac(isp,iespc,ident,spc_name)
...
elseif  (trim(chemical_mechanism) == 'CB07') then 
   ...
   !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_CB07(isp,iespc,ident,spc_name)
   !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_CB07(isp,iespc,ident,spc_name)
   return
...
elseif (trim(chemical_mechanism) == 'RELACS' .or. trim(chemical_mechanism) == 'RELACS_MX') then
   ...
   if (pond == 0) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_relacs(isp,iespc,ident,spc_name)      
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_relacs(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_relacs(isp,iespc,ident,spc_name)
   elseif (pond == 1) then
      ...
      !KYif(trim(inventory_type) == 'bioge')   call convert_bioge_to_relacs_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'edgar')   call convert_edgar_to_relacs_reac(isp,iespc,ident,spc_name)
      !KYif(trim(inventory_type) == 'megan')   call convert_megan_to_relacs_reac(isp,iespc,ident,spc_name)
   endif
   return

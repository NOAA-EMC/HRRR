...
!
! Modified by G.Pereira to assimiliate FEER (Ichoku product), Ichoku and Luke,
! 2014
! adapted to HRRR-Smoke in February 2017
!###############################################################################!
...
555     continue
   ...
   subroutine get_emission_per_fire_count(nfocos,qsc,qfires,qlon,qlat,qarea,qfrpw,qflam &
     ...
     integer :: nfocos,ifoc,i,iesp,nc,iespppp2
...
! ALTERADO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                biomass_burned =  qfre(ifoc)*1.37! kg[dry biomass burned]

! COEFICIENTE FEER PARA TPM
! Modified by G.Pereira to include the FEER approach to calculate PM10 emissions
! from Ichoku and Ellison, 2014
...
! ALTERADO !!!!!!!!!!!!!!!!!!!!!!
! TPM- total particulate matter in kg
! From TPM to all the chemical species

                        qsc(ifoc,iesp) =  biomass_burned * (emission_factor(iveg_agreg(ifoc),iesp)/emission_factor(iveg_agreg(ifoc),iespppp2)) !kg[spc]
...
!     call readOlsonHdf(nLon, nLat, olsonData)
     ...
     write(0,*) 'read olson binary file',trim(olson_data_dir)//'.fbin'
     open(30303,file=trim(olson_data_dir)//'.fbin',form='unformatted',status='old')
     read(30303) nlon,nlat
     allocate(olsonData(nLon, nLat))
     read(30303) olsonData
     close(30303)
...
subroutine c_abort
  ! Raise SIGABRT.  This typically exits the program and dump core.
  implicit none
  interface

...
! 180 / resolution of data in degrees
   ...
   INTEGER, PARAMETER ::  a_cols= 2951
   INTEGER, PARAMETER ::  a_rows= 1475
...
!      WRITE(*,*) 'cab ',cab
   ...
   resx = 0.122    ! IGBP land-use data spatial resolution
   resy = 0.122
...
! In near future this variable will be direct assessed in prep_chem.inp
   ...
   delta = 13.545 / 111.12  ! the model resolution is hard wired for the RAP res.
   ac_frp(:,:) = 0.
   ac_parea(:,:) = 0.
   cont(:,:) = 0

! A sample S-NPP FRP file
!year,month,day,hh,mm,lon,lat,mask,confidence,bright_t13,frp,line,sample,bowtie;    nfire = 54
!2018, 09, 18, 15, 29,  -38.798233,   -4.867128,   8,  64,  322.104858,   10.432212,    25,  2622,   0
...
! From Wilfried: You can assume MODIS 1km FRP to peak around 13,000MW, and VIIRS 750m FRP to peak at 16,000MW
   ...
   check_loc: IF ( (lat_vi .GT. -9.9) .AND. (lat_vi .LT. 89.9) ) THEN
    ...
    check_lulc:  IF (read_lulc==1) THEN
                    ...
                    print *, '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
                    print *, ' Processing S-NPP FRP data now', input_viirs

...
! Convert km to degrees;    In near future this variable will be entered in prep_chem.inp
        delta = 13.545 / 111.12        ! this needs to be updated in future to account for the latitude
        ac_frp(:,:) = 0.
        ac_area(:,:) = 0.
        cont(:,:) = 0
        ac_fsize(:,:) = 0.
...
! FORMAT >>  YYYYDDDHHMM_mod_3km.txt
                output = julday // chh // cmm // '_mod_RAP.txt'
...
! You can assume MODIS 1km FRP to peak around 13,000MW, and VIIRS 750m FRP to peak at 16,000MW
 ...
 check_loc:    IF ( (lat_m > 20.0) .AND. (lat_m < 55.0) .AND. (lon_m > -180.0) .AND. (lon_m < -30.0) ) THEN

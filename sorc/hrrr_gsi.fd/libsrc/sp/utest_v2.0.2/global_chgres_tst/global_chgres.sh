...
#  Directories.
export NWPROD=${NWPROD:-/gpfs/td1/global/noscrub/Shrinivas.Moorthi/para}
...
#export CHGRESEXEC=${CHGRESEXEC:-./global_chgres$XC}
...
#==EM==export OMP_NUM_THREADS=${OMP_NUM_THREADS_CH:-${CHGRESTHREAD:-16}}
export OMP_NUM_THREADS=${OMP_NUM_THREADS_CH:-${CHGRESTHREAD:-${MYTHREADS}}}
export CHGRESEXEC=./global_chgres
#if [ $machine = IBM ] ; then
echo $(date) "============# EM:: of Threads Requested" $OMP_NUM_THREADS
 ...
 /
EOF
echo $(date) "====AFTER========# EM:: of Threads Requested" $OMP_NUM_THREADS
#else
